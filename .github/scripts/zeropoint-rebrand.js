/**
 * oreo-blog Template Rebranding Script
...
async function rebrand() {
  ...
  // Safety check - ensure we're working with a oreo-blog template
  const packageJson = JSON.parse(readFileSync('package.json', 'utf8'));
  if (packageJson.name !== 'oreo-blog') {
    // Clean up template marker if this isn't a oreo-blog template
    unlinkSync('.template');
    if (isCI) {
      console.log('⚠️ Not a oreo-blog template, skipping...');
    }
    process.exit(0);
  }

  console.log('🪐 Welcome to oreo-blog! Let\'s customize this for your project.\n');
  ...
  replaceInFiles('oreo-blog', projectName.replace(/\s+/g, '')); // Remove spaces from project name
  replaceInFiles('Newman', author); // Replace default author
  replaceInFiles('oreo-blog', projectName.toLowerCase().replace(/\s+/g, '-')); // Lowercase with hyphens
  ...
  if (existsSync('README.oreo-blog.md')) {
    try {
      copyFileSync('README.oreo-blog.md', 'README.md'); // Use oreo-blog README as new README
    } catch (error) {
      console.warn(`Could not copy README.oreo-blog.md: ${error.message}`);
    }
  }
